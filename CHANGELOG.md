...
- **Download rules settings** - added network gating, concurrent-download limits, default audio bitrate, default thumbnail sidecar saving, and persistent playlist queue behavior inside Settings
- **Settings diagnostics tools** - added one-tap log export, log clearing, and app/runtime info copying so bug reports can include cleaner environment details
...
- **Queue scheduling behavior** - downloads now stage in-app until both a queue slot and the selected network type are available, so concurrency limits and Wi-Fi/unmetered rules work together more predictably
- **Cache maintenance defaults** - Settings now expose automatic temporary-cache cleanup windows instead of relying only on manual cache clearing
